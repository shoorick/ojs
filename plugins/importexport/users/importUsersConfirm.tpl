...
{foreach name=users from=$users item=user key=userKey}
	<tr valign="top">
		...
		<td><input type="text" name="{$userKey|escape}_middleName" value="{$user->getLocalizedMiddleName()|escape}" size="9" class="textField" /></td>
		<td><input type="text" name="{$userKey|escape}_lastName" value="{$user->getLocalizedLastName()|escape}" size="9" class="textField" /></td>
